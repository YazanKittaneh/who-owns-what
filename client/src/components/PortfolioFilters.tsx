...
const PortfolioFiltersWithoutI18n = React.memo(
  React.forwardRef<HTMLDivElement, PortfolioFilterProps>((props, ref) => {
    ...
    const [commercialOnlyActive, setCommercialOnlyActive] = React.useState(false);
    const updateCommercialOnly = () => {
      logPortfolioAnalytics(commercialOnlyActive ? "filterCleared" : "filterApplied", {
        column: "commercialOnly",
      });
      setCommercialOnlyActive(!commercialOnlyActive);
      setFilterContext({
        ...filterContext,
        filterSelections: {
          ...filterContext.filterSelections,
          commercialOnly: !filterContext.filterSelections.commercialOnly,
        },
      });
    };
    ...
    return (
      <div className="PortfolioFilters" ref={ref}>
        <div className="filters-container">
          ...
          >
            <button
              aria-pressed={commercialOnlyActive}
              onClick={updateCommercialOnly}
              className="filter filter-toggle"
              aria-label={i18n._(t`Commercial buildings only filter`)}
            >
...
function getUnitsresOptions(addrs: AddressRecord[]) {
  const allUnitsres = addrs.map((addr) => addr.unitsres ?? addr.units_res ?? 0);
