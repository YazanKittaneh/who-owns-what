...
base_candidates AS (
    (
        ...
        CROSS JOIN params
        WHERE o.prop_address_full IS NOT NULL
          AND params.query <> ''
          AND lower(o.prop_address_full) LIKE params.query || '%%'
        LIMIT 15
    )
    ...
    (
        ...
        WHERE o.prop_address_full IS NOT NULL
          ...
          AND o.prop_address_full ILIKE '%%' || params.query || '%%'
        LIMIT 25
    )
    ...
    (
        ...
        CROSS JOIN params
        WHERE o.prop_address_full IS NOT NULL
          AND params.query_street_body <> ''
          AND char_length(params.query_street_body) >= 2
          AND o.prop_address_full ILIKE '%%' || params.query_street_body || '%%'
        LIMIT 40
    )
    ...
    (
        ...
        CROSS JOIN params
        WHERE o.prop_address_full IS NOT NULL
          AND params.query_housenumber_prefix IS NOT NULL
          AND params.query_direction IS NOT NULL
          AND lower(o.prop_address_full) LIKE params.query_housenumber_prefix || '%%'
          AND lower(o.prop_address_full) LIKE '%% ' || params.query_direction || ' %%'
        LIMIT 40
    )
),
...
address_prefix AS (
    ...
    FROM (
        SELECT
            ...
            0 AS direction_rank,
            row_number() OVER (ORDER BY na.address) AS row_num
        FROM normalized_addresses AS na
        CROSS JOIN params
        WHERE params.query <> ''
          AND na.normalized_address LIKE params.query || '%%'
    ) AS ranked
    CROSS JOIN params
...
nearby_housenumber AS (
    SELECT
        ...
        na.zip,
        1 AS priority,
        abs(na.housenumber_num - params.query_housenumber_num) AS house_number_distance,
        CASE
            WHEN params.query_direction IS NULL THEN 0
            WHEN na.street_direction = params.query_direction THEN 0
            WHEN na.street_direction IS NULL THEN 1
            ELSE 2
    ...
    WHERE params.query <> ''
      ...
      AND na.housenumber_num IS NOT NULL
      AND abs(na.housenumber_num - params.query_housenumber_num) <= 4
      AND (
          (
              params.query_street_body <> ''
              AND (
                  na.street_body LIKE params.query_street_body || '%%'
                  OR (
                      char_length(params.query_street_body) >= 2
                      AND na.street_body LIKE '%%' || params.query_street_body || '%%'
                  )
              )
          )
          OR (
              params.query_street_body = ''
              AND params.query_direction IS NOT NULL
              AND na.street_direction = params.query_direction
          )
      )
    LIMIT 5
),
...
street_contains AS (
    ...
    WHERE params.query_street_body <> ''
      AND na.street_body LIKE '%%' || params.query_street_body || '%%'
      AND na.street_body NOT LIKE params.query_street_body || '%%'
    LIMIT 5
),
address_contains AS (
    SELECT
    ...
    WHERE params.query <> ''
      AND na.normalized_address LIKE '%%' || params.query || '%%'
      AND na.normalized_address NOT LIKE params.query || '%%'
    LIMIT 5
),
candidates AS (
