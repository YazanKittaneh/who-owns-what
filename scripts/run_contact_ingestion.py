...
def main():
    ...
    parser.add_argument(
        "--skip-sos", action="store_true", help="Skip SOS contact extraction."
    )
    parser.add_argument(
    ...
    if not args.skip_building_permits:
        if table_has_rows(database_url, "chi_permits"):
            ...
            print(
                "Skipping building permit extraction (table missing or empty: chi_permits)"
            )
    ...
    if not args.skip_foreclosed_rental:
        if table_has_rows(database_url, "chi_foreclosed_rental_properties"):
            ...
            print(
                "Skipping foreclosed rental extraction (table missing or empty: chi_foreclosed_rental_properties)"
            )
    ...
    print("\nNext steps:")
    print(
        "  - Verify data quality with: curl http://localhost:8000/api/admin/contact-coverage"
    )
    print(
        "  - Search entities: curl 'http://localhost:8000/api/entity/search?q=example'"
    )
    print(
        "  - Get entity contacts: curl 'http://localhost:8000/api/entity/contacts?entity_id=1'"
    )
