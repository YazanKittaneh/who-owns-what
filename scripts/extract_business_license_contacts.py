#!/usr/bin/env python3
"""
Load contact data from chi_business_licenses using SQL-first bulk operations.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Load contacts from chi_business_licenses with SQL-first bulk operations."
    )
    parser.add_argument(
        "--data-dir",
        default="data/supplemental-20260329",
        help="Deprecated. Kept for compatibility; loader reads from chi_business_licenses in Postgres.",
    )
    parser.add_argument(
        "--link-parcels",
        action="store_true",
    ...
    run_id = build_run_id("contact_ingest_business_licenses")
    ...
    try:
        ...
        record_load_audit(
            conn,
            dataset_name="contact_ingest_business_licenses",
            table_name="entity_contacts",
            row_count=inserted_contacts,
            source_ref="chi_business_licenses",
            run_id=run_id,
            status="success",
            details=details,
        )
        ...
        print("=" * 60)
        print("BUSINESS LICENSE CONTACT LOAD COMPLETE")
        print("=" * 60)
