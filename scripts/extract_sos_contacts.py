...
class SOSContactExtractor:
    ...
    def ingest_sos_entities(self, data_dir: Path) -> None:
        ...
        if corp_agents_path.exists():
            with corp_agents_path.open('r', newline='', encoding='utf-8', errors='replace') as f:
        ...
        if llc_agents_path.exists():
            with llc_agents_path.open('r', newline='', encoding='utf-8', errors='replace') as f:
                reader = csv.DictReader(f)
                for row in reader:
        ...
        if corp_master_path.exists():
            with corp_master_path.open('r', newline='', encoding='utf-8', errors='replace') as f:
                reader = csv.DictReader(f)
                for row in reader:
        ...
        if llc_master_path.exists():
            with llc_master_path.open('r', newline='', encoding='utf-8', errors='replace') as f:
                reader = csv.DictReader(f)
                for row in reader:
    ...
    def ingest_to_database(self, entities: Dict[str, SOSEntity]) -> None:
        ...
        with self.conn.cursor() as cursor:
            for file_number, entity in entities.items():
                source_system = self.SOURCE_CORP if entity.entity_type == 'corporation' else self.SOURCE_LLC
        ...
        print(f"Ingested {self.stats['entities_matched']} entities with {self.stats['contacts_extracted']} contacts")
